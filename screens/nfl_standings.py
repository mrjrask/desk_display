...
def _render_conference(title: str, division_order: list[str], standings: dict[str, list[dict]]) -> Image.Image:
    ...
    for division, section_height in zip(division_order, sections, strict=True):
        ...
        for team in teams:
            abbr = team.get("abbr", "")
            display_text = _team_display_name(team) or abbr
...
def draw_nfl_overview_nfc(
    ...
    standings_by_conf, fallback_message, season_note = (
        ...
        else (standings, fallback_message, season_note)
    )

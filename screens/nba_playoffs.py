...
SCOREBOARD_LOGO_BASE_HEIGHT = scale_value_width(36) if HYPERPIXEL_LAYOUT else scale_value_width(52)
...
def _extract_next_game_dt(value: Any) -> Optional[datetime.datetime]:
    if isinstance(value, (int, float)):
        try:
            return datetime.datetime.fromtimestamp(float(value), tz=datetime.UTC).astimezone(CENTRAL_TIME)
        except Exception:
            return None
...
_EASTERN_CONFERENCE_ABBRS = frozenset(
    {
        "ATL", "BOS", "BKN", "BRK", "CHA", "CHI", "CLE", "DET", "IND",
        "MIA", "MIL", "NY", "NYK", "ORL", "PHI", "TOR", "WAS", "WSH",
    }
)
_WESTERN_CONFERENCE_ABBRS = frozenset(
    {
        "DAL", "DEN", "GS", "GSW", "HOU", "LAC", "LAL", "MEM", "MIN",
        "NO", "NOP", "OKC", "PHX", "POR", "SAC", "SA", "SAS", "UTA",
    }
)

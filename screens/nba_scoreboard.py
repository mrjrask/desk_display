...
INTRO_ANIM_DELAY = max(0.06, DISPLAY_ANIMATION_FRAME_INTERVAL)
INTRO_ANIM_HOLD = 0.4
...
def _intro_max_height() -> int:
    team_scale = get_screen_image_scale(SCREEN_ID, "team_logo", 1.0)
    scale = get_screen_image_scale(SCREEN_ID, "intro_logo", team_scale)
...
def _render_intro_frame(logo: Image.Image, scale: float) -> Image.Image:
    ...
    intro_max = _intro_max_height()
    max_height_scale = (
        intro_max / logo.height if logo.height else base_scale
    )

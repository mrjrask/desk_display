...
SETTINGS: tuple[Setting, ...] = (
    ...
    _s("DESK_DISPLAY_CLIENT_TOUCH", "choice", _CLIENT_ONLY, "touch",
       "Whether this client has a touch panel; auto assumes one on HyperPixel profiles.",
...
_EXAMPLE_INTROS = {
    ...
    Role.CLIENT: (
        ...
        "Copy to .env.client on each display: the client service reads that file, so",
        "a combined install keeps the server's .env beside it. A client needs only",
        "its identity, the server URL, its credential and its own hardware settings:",
        "never an upstream API key or provider URL. The installer writes this file",
        "(see OPERATIONS.md). Check a file with:",

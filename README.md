...
| Variable | Description |
| --- | --- |
| `SCREEN_CONFIG_HOST` | Config UI bind host; default is `0.0.0.0` so the UI is reachable from other devices on the network. Set it to `127.0.0.1` to restrict access to the display itself. |
...
Because the Config UI is network-accessible by default, configure
`SCREEN_UI_PASSWORD` on networks where other users should not have access.

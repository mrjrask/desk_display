...
def fetch_air_quality(latitude: float, longitude: float, *, include_pollen: bool = True, timeout: float = 8.0) -> Optional[AirQualityReport]:
    """Fetch current air quality, retaining pollen data for supported IQAir locations."""

    iqair_url = iqair_url_for_coordinates(latitude, longitude)
    if iqair_url:
        report = fetch_iqair_air_quality(iqair_url, timeout=timeout)
        if report is not None:
            if not include_pollen:
                return report
            pollen_report = _fetch_open_meteo_air_quality(
                latitude, longitude, include_pollen=True, timeout=timeout
            )
            if pollen_report is None:
                return report
            return replace(
                report,
                pollen_level=pollen_report.pollen_level,
                advisory_text=advisory_for(report.aqi_category, pollen_report.pollen_level),
            )
        logging.warning("Falling back to Open-Meteo AQI for %s, %s", latitude, longitude)

    return _fetch_open_meteo_air_quality(
        latitude, longitude, include_pollen=include_pollen, timeout=timeout
    )

...
def test_build_image_prefers_reverse_split_symbol_then_fallback(monkeypatch):
    ...
    def _fake_fetch(symbol, display_symbol=None):
        calls.append((symbol, display_symbol))
        if symbol == draw_vrnof.VRNO_PRIMARY_QUOTE_SYMBOL:
        ...
        draw_vrnof._cache.update({
            ...
            "all_time": "-7.10%",
            "ts": time.time(),
            "active_symbol": display_symbol or symbol,
        })
        return True
    ...
    assert calls == [
        (draw_vrnof.VRNO_PRIMARY_QUOTE_SYMBOL, draw_vrnof.VRNO_PRIMARY_SYMBOL),
        (draw_vrnof.VRNO_FALLBACK_SYMBOL, draw_vrnof.VRNO_FALLBACK_SYMBOL),
    ]
...
def test_build_image_displays_reverse_split_symbol_when_available(monkeypatch):
    ...
    assert calls == [(draw_vrnof.VRNO_PRIMARY_QUOTE_SYMBOL, draw_vrnof.VRNO_PRIMARY_SYMBOL)]

...
def test_fetch_air_quality_uses_open_meteo_for_all_coordinates(monkeypatch):
    ...
    def fake_request_json(url, *, params, timeout, quiet):
        requested.update(url=url, params=params, timeout=timeout, quiet=quiet)
        return {"current": {"us_aqi": 55, "pm2_5": 10.0}, "hourly": {"us_aqi": [55, 54]}}
    ...
    assert requested["url"] == air_quality.OPEN_METEO_AIR_QUALITY_URL
    assert requested["params"]["latitude"] == 42.1373
    assert requested["params"]["longitude"] == -87.8446

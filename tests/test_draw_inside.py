...
def test_get_smbus_candidates_does_not_force_primary_bus_first(monkeypatch):
    import screens.draw_inside as draw_inside_module

    monkeypatch.setattr(draw_inside_module, "_resolve_i2c_bus_number", lambda _i2c: 13)
    monkeypatch.setattr(draw_inside_module, "_parse_i2c_bus_candidates", lambda: (13, 14, 15))
    monkeypatch.setattr(
        draw_inside_module,
        "_i2cdetect_bus_has_known_sensor",
        lambda bus_num: bus_num == 15,
    )

    assert draw_inside_module._get_smbus_candidates(object()) == (15, 13, 14)

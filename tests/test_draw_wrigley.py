...
def test_download_wrigley_frame_skips_earthcam_generic_placeholder(monkeypatch):
    html = """
    <html>
      <body>
        <img src="https://static.earthcam.com/images/placeholder_generic_cam.jpg" />
        <img src="https://media.example.com/wrigley/live_snapshot.jpg" />
      </body>
    </html>
    """
    ...
    def _fake_get(url, timeout=None, headers=None):  # noqa: ARG001
        if url == draw_wrigley.WRIGLEY_PAGE_URL:
            return _Resp(text=html, content_type="text/html")
        if "placeholder_generic_cam.jpg" in url:
            return _Resp(content=_img_bytes((1280, 720)), content_type="image/jpeg")
        if "live_snapshot.jpg" in url:
            return _Resp(content=_img_bytes((1280, 720)), content_type="image/jpeg")
        raise AssertionError(f"Unexpected URL: {url}")
    ...
    assert image is not None
    assert source == "https://media.example.com/wrigley/live_snapshot.jpg"

...
def test_hourly_night_cutover_during_standard_time():
    ...
    assert _is_night_time_hourly(_to_ts(2026, 1, 10, 16), 0, 0, increment_seconds=3600) is False
    assert _is_night_time_hourly(_to_ts(2026, 1, 10, 17), 0, 0, increment_seconds=3600) is True


def test_hourly_switches_back_to_daytime_after_5am():
    assert _is_night_time_hourly(_to_ts(2026, 1, 10, 4), 0, 0, increment_seconds=3600) is True
    assert _is_night_time_hourly(_to_ts(2026, 1, 10, 5), 0, 0, increment_seconds=3600) is False

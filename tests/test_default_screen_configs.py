...
# Approved scheduler cadence snapshots. Each outer-list position is a normal
# pass (1 through 12); startup hydration is deliberately excluded. Keeping the
# literal matrices here makes cadence and ordering failures directly reviewable.
...
def _normal_pass_matrix(config: dict, pass_count: int = 12) -> list[list[str]]:
    ...
    # Startup plus each normal pass can emit no more than one item per configured
    # base screen. This bound therefore always reaches the requested pass count.
